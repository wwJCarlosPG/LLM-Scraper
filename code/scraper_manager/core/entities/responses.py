...
class ScrapedResponse(BaseModel):
    ...
    Attributes:
        explanation (str):
            ...
            the extracted data or process. Defaults to "No explanation".
        ...
        feedback (str):
            A detailed justification of the validation outcome,
            describing why the extracted data is considered correct or incorrect
            based on the provided HTML content and query. Defaults to None.

        is_valid (bool):
            A boolean flag indicating the validation result.
        ...
        refinement_count (int):
            The number of times the extracted data has been refined to improve accuracy.
            Defaults to 0.
    """
...
class Response(BaseModel):
    ...
    This class can hold either a single `ScrapedResponse` or a list of multiple
    `ScrapedResponse` objects. It is designed to structure the results of the
    web scraping process and make it easier to manage multiple potential outputs
    based on different interpretations or extractions.

    Attributes:
        responses (Union[ScrapedResponse, list[ScrapedResponse]]):
            A single `ScrapedResponse` or a list of `ScrapedResponse` objects that
            represent the extracted data from the web scraping process. Each
            `ScrapedResponse` object contains details about the data, such as
